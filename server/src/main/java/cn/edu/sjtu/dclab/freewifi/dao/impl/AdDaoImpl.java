...
public class AdDaoImpl implements IAdDao {
	...
	public List<Ad> getLaunchingAdByMerchantAndUser(Merchant merchant, User user) {
		...
		//添加七个定向方案
		criteria.createAlias("orientation", "ori");
		criteria.add(Restrictions.ilike("ori.sex", ""+user.getSex().getValue(), MatchMode.ANYWHERE));
		criteria.add(Restrictions.ilike("ori.income", ""+user.getIncome().getValue(), MatchMode.ANYWHERE));
		criteria.add(Restrictions.ilike("ori.education", ""+user.getEducation().getValue(), MatchMode.ANYWHERE));	
		criteria.add(Restrictions.ilike("ori.age", ""+AgeType.getByBirthDate(user.getBirthdate()).getValue(), MatchMode.ANYWHERE));
		criteria.add(Restrictions.ilike("ori.engage", ""+user.getEngage().getValue(), MatchMode.ANYWHERE));
		criteria.add(Restrictions.ilike("ori.baby", ""+user.getBaby().getValue(), MatchMode.ANYWHERE));
		criteria.add(Restrictions.ilike("ori.job", ""+user.getJob().getValue(), MatchMode.ANYWHERE));
		//时间配置
